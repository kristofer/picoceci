...
13. [Interop with TinyGo / Device Services](#13-interop-with-tinygo--device-services)
...
| `DeviceError` | Built-in device service failure (`Wifi`, `SDCard`, `LED`, GPIO, etc.) |
...
## 13. Interop with TinyGo / Device Services
...
### 13.2 Built-in singleton services

picoceci v3 hosts MCU services directly in the runtime as singleton objects. These are globally visible objects (like `Console`) and are safe to use from `Task` blocks.

```picoceci
Wifi connectSSID: 'ship-net' password: 'orbital-1234'.
SDCard mount.
LED blinkEvery: 250.
```

Required singleton objects:

| Object | Core responsibilities |
|---|---|
| `Wifi` | WiFi station management and TCP listener/session setup |
| `SDCard` | Mount state, card health/status, root path access |
| `LED` | Board status indicator and heartbeat controls |

### 13.3 WiFi/TCP entry point
...
`Wifi listenOn:do:` is the network ingress primitive for remote interaction with a running picoceci VM on ESP32-S3.
